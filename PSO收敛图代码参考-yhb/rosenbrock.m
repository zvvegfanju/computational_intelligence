...
% , 2000, 186(2):311-38. 
%------------------------------------
% parent selection: 2-tournament selection 二元锦标赛选择
% crossover: simulated binary crossover 模拟二进制交叉
% mutation: ploynomial mutation 多项式变异
% using environment selection
%------------------------------------
clearvars;clc;close all;
%------------------------------------
% problem name 'Fun_Ellipsoid','Fun_Rosenbrock', 'Fun_Ackley', 'Fun_Griewank','Fun_Test'
figure
record=[]
for i =1:20
...
case 'Fun_Test'
lower_bound = -5*ones(1,num_vari); upper_bound = 5*ones(1,num_vari);
end
...
pop_parent = pop_vari(sum(temp.*(index == 1:k),2),:); % 选择竞标赛选择中获胜的也即适应度较小的个体作为父代交叉个体
%------------------------------------
...
betaq(rand(pop_size/2,num_vari)>0.7) = 1; % 父代两两交叉，设置一般父代的交叉概率
...
dis_m = 100;
pro_m = 1/num_vari; % 变异概率
rand_var = rand(pop_size,num_vari); % 产生每个个体的变异概率
